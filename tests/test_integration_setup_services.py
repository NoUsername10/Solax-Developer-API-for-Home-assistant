...
from custom_components.solax_developer_api import feature_services as feature_services_module
...
from custom_components.solax_developer_api.feature_services import FeatureServiceHandlers
...
@pytest.mark.asyncio
async def test_feature_service_dispatch_skips_unloaded_entries_and_cancels_targets(
    monkeypatch,
):
    coordinator = _Coordinator()
    loaded = _entry(coordinator, entry_id="loaded")
    unloaded = _entry(
        entry_id="unloaded",
        state=ConfigEntryState.NOT_LOADED,
    )
    hass = _Hass([unloaded, loaded])

    def _loaded_runtime(_hass, entry_id):
        if entry_id == "loaded":
            return loaded.runtime_data
        return None

    def _resolve(_hass, _call):
        return "loaded", coordinator

    def _error(key, **_kwargs):
        return ValueError(key)

    handlers = FeatureServiceHandlers(
        hass,
        loaded_runtime=_loaded_runtime,
        resolve_coordinator=_resolve,
        translated_error=_error,
    )

    history = await handlers.async_list_history_devices(_call())
    plants = await handlers.async_list_plant_statistics_targets(_call())
    alarms = await handlers.async_list_alarm_targets(_call())
    assert history["entries"] == ["loaded"]
    assert plants["entries"] == ["loaded"]
    assert alarms["entries"] == ["loaded"]

    explicit = await handlers.async_cancel_fetch(
        _call(entry_id="loaded", request_id="explicit-request")
    )
    assert explicit["entries"]["loaded"]["request_id"] == "explicit-request"

    global_result = await handlers.async_cancel_fetch(
        _call(request_id="global-request")
    )
    assert list(global_result["entries"]) == ["loaded"]
    assert global_result["request_id"] == "global-request"

    with pytest.raises(ValueError, match="plant_year_invalid"):
        await handlers.async_fetch_plant_year_statistics(
            _call(plant_id="P1", business_type=1, year=1999)
        )

    monkeypatch.setattr(
        feature_services_module.dt_util,
        "now",
        lambda: datetime(2026, 4, 15),
    )
    with pytest.raises(ValueError, match="plant_month_invalid"):
        await handlers.async_fetch_plant_month_statistics(
            _call(plant_id="P1", business_type=1, year=2026, month=5)
        )

    forwarded = await handlers.async_fetch_plant_month_statistics(
        _call(
            plant_id=" P1 ",
            business_type=4,
            year=2025,
            month=12,
            request_id=" month-request ",
        )
    )
    assert forwarded["plant_month"] == {
        "plant_id": "P1",
        "business_type": 4,
        "year": 2025,
        "month": 12,
