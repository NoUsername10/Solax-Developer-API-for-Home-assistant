...
def test_sanitize_fully_redacts_nested_ev_charger_access_data():
    payload = {
        "ev_charger_control_commands": [
            {
                "payload": {
                    "ocppUrl": "wss://private.example/ocpp",
                    "ocpp_charger_id": "private-charger-id",
                    "qrCode": "private-qr-value",
                },
                "response": {
                    "result": {
                        "ocpp_url": "wss://response.example/ocpp",
                        "qr_code": "response-qr-value",
                    }
                },
            }
        ]
    ...
    for secret in (
        "wss://private.example/ocpp",
        "private-charger-id",
        "private-qr-value",
        "wss://response.example/ocpp",
        "response-qr-value",
    ):
        assert secret not in serialized
    assert serialized.count(REDACTED_VALUE) == 5


def test_config_entry_snapshot_uses_options_first_for_migrated_settings():
    entry = SimpleNamespace(
        ...
        data={
            ...
            "system_name": "Legacy Name",
            "scan_interval": 120,
            "api_region": "eu",
        },
        options={"system_name": "Current Name", "scan_interval": 300},
    )
    client = SimpleNamespace(access_token=None)
    snapshot = _build_config_entry_snapshot(entry, {}, client)
    assert snapshot["system_name"] == "Current Name"
    assert snapshot["scan_interval"] == 300
